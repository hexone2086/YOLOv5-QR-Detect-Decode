...
// std::vector<Detection> postprocess(const std::vector<float>& output, float
...
//         printf("Detection %zu: x=%.2f, y=%.2f, w=%.2f, h=%.2f, obj_conf=%.2f,
//         cls_conf=%.2f, confidence=%.2f\n",
...
int main(int argc, char **argv) {
  if (argc < 3) {
    std::cerr << "Usage: " << argv[0] << " <yolov5s_model_path>"
              << " <fsrcnn_model_path>" << "<image_path_or_camera_index>"
              << std::endl;
    return 1;
  }
  ...
  runInference(yolo_weightsPath, source);
  return 0;
}